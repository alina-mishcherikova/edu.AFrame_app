AFRAME.registerComponent("selection-menu", {
  init() {
    ...
    if (this.el.sceneEl.hasLoaded) setTimeout(this.buildMenu, 200);
    else
  ...
  toggleMenu(open) {
    this.menu.setAttribute("visible", open ? "true" : "false");
    if (open) {
      let sceneEl = this.el.sceneEl;
      if (typeof THREE === "undefined") return;

      // Use head as camera position reference
      let threeCamera =
        (sceneEl.renderer &&
          sceneEl.renderer.xr &&
      ...
      if (threeCamera) {
        ...
        threeCamera.getWorldPosition(worldPos); // vector of camera position in world space
        threeCamera.getWorldDirection(forward); // vector of camera forward direction
        const dist = 0.8;
